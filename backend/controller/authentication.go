...
import (
	...
	"github.com/Szeenom/sa-65-felix/service"
...
func Login(c *gin.Context) {
	...
	if err := entity.DB().Raw("SELECT * FROM students WHERE STUDENT_NUMBER = ?", payload.Username).Scan(&user).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	...
	signedToken, err := jwtWrapper.GenerateToken(user.STUDENT_NUMBER)
	if err != nil {
